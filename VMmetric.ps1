...
foreach ($vm in $virtualMachines) {
    ...
    $metricNames = @("Percentage CPU", "Available Memory Bytes", "VmAvailabilityMetric")
    ...
    foreach ($metricName in $metricNames) {
        ...
        try {
            ...
            elseif ($metricName -eq "VmAvailabilityMetric") {
                # Calculate the average availability percentage directly
                $averageAvailability = $totalMetricCount / $vmMetrics.Count
                #Round to 1 if greater than 0.5
                if ($averageAvailability -gt 0.5) {
                     $averageAvailability = 1
                    }
                    else {
                        $averageAvailability = 0
                    }
                $metricsData | Add-Member -MemberType NoteProperty -Name $metricName -Value $averageAvailability -Force
            }
...
$csvFilePath = "$outputpath\VMmetric.csv"
